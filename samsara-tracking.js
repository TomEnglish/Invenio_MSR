...
function initializeMap() {
    // Create map centered on project site
    ...
    const siteIcon = L.divIcon({
        ...
        html: '<div style="background-color: #8dc63f; width: 20px; height: 20px; border-radius: 50%; border: 3px solid white; box-shadow: 0 2px 8px rgba(0,0,0,0.3);"></div>',
        iconSize: [20, 20],
        iconAnchor: [10, 10]
    });
    ...
    state.siteMarker = L.marker([SITE_LAT, SITE_LON], { icon: siteIcon })
        .addTo(state.map)
        .bindPopup('<b>Project X Site</b><br>Project Site Location');
...
function addTrackerMarker(tracker) {
    ...
    if (tracker.status === 'On Site') color = '#8dc63f';  // Lime green
    ...
    const popupContent = `
        <div style="min-width: 200px;">
            <h6 style="margin: 0 0 10px 0; color: #3a3a3a;">${tracker.name}</h6>
